...
# ---------------------------------------------------------------------------
_POLLINATIONS_URL = "https://image.pollinations.ai/prompt/{prompt}?width=1024&height=1024&nologo=true&model=flux&seed={seed}"


async def _generate_image(prompt: str, seed: int | None = None) -> discord.File | None:
    import random as _random
    s = seed if seed is not None else _random.randint(1, 999999)
...
# ---------------------------------------------------------------------------
def _sim_embed(data: dict[str, Any], description: str) -> discord.ui.Container:
...
class Fabrication(commands.Cog):
    ...
    async def project(self, ctx: commands.Context, *, description: str):
        """Render a holographic blueprint via DALL-E 3."""
        ...
        if file is None:
            await v2.edit(status, v2.warn(
                "⬡  Array Offline",
                "Holographic projection is temporarily unavailable — the render service could not be reached. Try again in a moment.",
            ))
            return
    ...
    async def rotate(self, ctx: commands.Context, *, description: str):
        """Generate three angle renders of the design in sequence."""
        status = await v2.send(ctx, v2.info(
        ...
        files_ready = [(label, f) for (label, _), f in zip(_ROTATION_ANGLES, renders) if f]

        if not files_ready:
            await v2.edit(status, v2.warn(
                "↻  Array Offline",
                "Holographic rotation is temporarily unavailable — the render service could not be reached. Try again in a moment.",
            ))
            return
    ...
    async def simulate(self, ctx: commands.Context, *, description: str):
        """Run fabrication simulation and display a full diagnostic readout."""
        status = await v2.send(ctx, v2.info(
        ...
        if data is None:
            await v2.edit(status, v2.danger(
                "◈  Simulation Failure",
                "Diagnostic AI returned an error. Verify that `MISTRAL_API_KEY` is configured in Render.",
            ))
            return
    ...
    @commands.hybrid_command(
        name="fabricate",
        description="Full Jarvis pipeline — holographic projection + live simulation simultaneously.",
    )
    async def fabricate(self, ctx: commands.Context, *, description: str):
        """Project schematic AND run simulation in parallel."""
        status = await v2.send(ctx, v2.info(
        ...
        if not has_image and not has_sim:
            await v2.edit(status, v2.danger(
                ...
                "Verify that `MISTRAL_API_KEY` is set in your Render environment variables.",
            ))
            return

        designation = sim_data.get("designation", description[:20].upper()) if sim_data else description[:20].upper()
        ...
        if has_sim:
            _lv = discord.ui.LayoutView(timeout=None)
            _lv.add_item(_sim_embed(sim_data, description))
            await ctx.channel.send(view=_lv)
